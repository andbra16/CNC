...
public class StaticTypeCheck {
    ...
    public static void V (Expression e, TypeMap tm) {
		...
		if (e instanceof Unary) {
			...
			V (u.term, tm);
			if (u.op.ArithmeticOp()) {
				check ((type == Type.INT || type == Type.FLOAT),
						"type error for " + u.op);
			} else if (u.op.intOp()) {
				check ((type == Type.FLOAT), "type error for " + u.op);
			} else if (u.op.charOp()) {
    ...
    public static void V (Statement s, TypeMap tm) {
        ...
        // student exercise
			...
			if (testtype == Type.BOOL) {
				...
				check(false, "type in Conditional isn't a bool: " + c.test);
			}
			...
			if (testtype == Type.BOOL) {
				...
				check(false, "type in Loop Cond" +
					"itional isn't a bool: " + l.test);
			}
